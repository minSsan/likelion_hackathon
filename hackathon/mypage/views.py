...
def detail_portfolio(request, user_id, pf_id):
    ...
    return render(request, 'mypage/detail_pf.html', context)
...
def create_portfolio(request, user_id): # user id 필요
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_portfolio.save()
        return redirect('mypage:profile', user_id) # 유저 페이지 redirect
    else:
        context = {
            'form':form,
            'user_id':user_id,
        }
        return render(request, 'mypage/create_pf.html', context)
...
def delete_portfolio(request, user_id, pf_id):
    ...
    pf_instance.delete()
    return redirect('mypage:profile', user_id)
...
def update_portfolio(request, user_id, pf_id):
    ...
    if request.method == "POST":
        ...
        return render(request, 'mypage/update_pf.html', context)
